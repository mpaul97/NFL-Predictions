...
class CoachElos:
    ...
    def update(self, isNewYear: bool):
        """
        Updates rawCoachElos.csv when gameData contains new/unseen wys
        @params:
        ...
        if new_last_wy != self.raw_elos['wy'].values[-1]:
            ...
            print(f"rawCoachElos.csv updated. Added elos for wys: {missing_wys}")
        else:
            print(f'rawCoachElos.csv already up-to-date. New last wy: {new_last_wy}')
        return
