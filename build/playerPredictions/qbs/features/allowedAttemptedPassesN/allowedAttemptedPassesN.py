...
def buildAllowedAttemptedPassesN(n):
    ...
    cols = [("allowedAttemptedPasses" + str(n) + "_" + str(i)) for i in range(n)]
    ...
    for index, row in source.iterrows():
        ...
        opps = opps.tail(n)
        home_stats = opps.loc[opps['home_abbr']==opp_abbr, 'away_pass_attempts']
        away_stats = opps.loc[opps['away_abbr']==opp_abbr, 'home_pass_attempts']
    ...
    df.to_csv("%s.csv" % ("allowedAttemptedPasses" + str(n)), index=False)

###############################

buildAllowedAttemptedPassesN(10)