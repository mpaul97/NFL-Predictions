...
def buildAllowedPassingYardsN(n):
    ...
    cols = [("allowedPassingYards" + str(n) + "_" + str(i)) for i in range(n)]
    ...
    for index, row in source.iterrows():
        ...
        opps = opps.tail(n)
        home_stats = opps.loc[opps['home_abbr']==opp_abbr, 'away_pass_yards']
        away_stats = opps.loc[opps['away_abbr']==opp_abbr, 'home_pass_yards']
    ...
    df.to_csv("%s.csv" % ("allowedPassingYards" + str(n)), index=False)

###############################

buildAllowedPassingYardsN(10)