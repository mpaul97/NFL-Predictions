...
def buildPassingTouchdownsN(n):

    source = pd.read_csv("%s.csv" % (SOURCE_PATH + "source"))
    cd = pd.read_csv("%s.csv" % (DATA_PATH + "QBData"))

    cols = [("passingTouchdowns" + str(n) + "_" + str(i)) for i in range(n)]
    ...
    for index, row in source.iterrows():
        ...
        if stats.empty:
            ...
            stats = stats['passing_touchdowns'].values
    ...
    df.to_csv("%s.csv" % ("passingTouchdowns" + str(n)), index=False)

###############################

buildPassingTouchdownsN(10)