...
def getFeaturesAndLabels(trainNum, targetName):
    ...
    keep_cols = ['key', 'opp_abbr', 'wy', 'p_id', targetName]
...
def alterData(features, labels, shorten, shortenYear, ols, olsThreshold, saveOls, fs):
    ...
    col_drops = ['key', 'opp_abbr', 'wy', 'p_id']
...
def predict(features, labels, modelName, trainTest, targetName, standardizeX, standardizeY):

    # standardize Y
    if standardizeY:
        maxY = labels[targetName].max()
        labels[targetName] = labels[targetName] / maxY
    else:
        maxY = 1
    ...
    for p in predictions:
        print(p[0] + " : " + str(p[1]*maxY) + " <-> Actual: " + str(p[2]*maxY))
...
targetName = 'passing_touchdowns'

features, labels = getFeaturesAndLabels(13, targetName)
