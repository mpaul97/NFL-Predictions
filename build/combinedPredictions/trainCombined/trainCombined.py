...
GAME_TARGET_PATH = "../../gamePredictions/targets/"
QBS_TARGET_PATH = "../../playerPredictions/qbs/targets/"

def getFeaturesAndLabels(suffix, targetType, targetName):
    
    features = pd.read_csv("%s.csv" % ("trainCombined_" + str(suffix)))
    
    if targetType == 'g':
        labels = pd.read_csv("%s.csv" % (GAME_TARGET_PATH + "target"))
    elif targetType == 'q':
        labels = pd.read_csv("%s.csv" % (QBS_TARGET_PATH + "target"))
...
def predict(features, labels, modelName, trainTest, targetName):
    ...
    if targetName != 'simplePoints':
        for p in predictions:
            print(p[0] + " : " + str(p[1]) + " <-> Actual: " + str(p[2]))
    else:
        for p in predictions:
            print(p[0] + " : " + str(p[1]*7) + " <-> Actual: " + str(p[2]*7))
...
targetType = 'q'
targetName = 'passing_yards'

features, labels = getFeaturesAndLabels(
    suffix="g14_qbs12_", 
