...
def buildPointsAllowedN(n):
    ...
    for index, row in source.iterrows():
        ...
        stats = cd.loc[
            (cd.index<start)&
            ((cd['home_abbr']==opp_abbr)|(cd['away_abbr']==opp_abbr))
        ]
        stats = stats.tail(n)
        if stats.empty:
            ...
            homePoints = stats.loc[stats['away_abbr']==opp_abbr, 'home_points']
            awayPoints = stats.loc[stats['home_abbr']==opp_abbr, 'away_points']
            points = pd.concat([homePoints, awayPoints]).sort_index(ascending=False).values
    ...
    df.to_csv("%s.csv" % ("pointsAllowed" + str(n)), index=False)
        
######################################

buildPointsAllowedN(10)