...
def replaceColsFast(temp, isHome, N):
    if isHome:
        ...
        for name in col_names:
            new_col_name = name.replace('home_', 'opp_' + str(N) + '_').replace('away_', 'opp_' + str(N) + '_opp_')
            temp = temp.rename(columns={name: new_col_name})
            # temp = temp.rename(index={temp.last_valid_index(): abbr})
    else:
        col_names = list(temp.columns)
        for name in col_names:
            new_col_name = name.replace('away_', 'opp_' + str(N) + '_').replace('home_', 'opp_' + str(N) + '_opp_')
            temp = temp.rename(columns={name: new_col_name})
            # temp = temp.rename(index={temp.last_valid_index(): abbr})
    return temp
...
def buildOppN(n):
    ...
    for index, row in source.iterrows():
        ...
        stats = cd.loc[
            ...
            ((cd['home_abbr']==oppAbbr)|(cd['away_abbr']==oppAbbr))
        ]
        stats = stats.tail(n)
        if stats.empty:
    ...
    source.to_csv("%s.csv" % ("opp" + str(n)), index=False)
